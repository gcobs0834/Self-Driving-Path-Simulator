...
class GUI(tk.Frame):
    def __init__(self, master):
        ...
        # self.set_mode('6D')
        self.ga = PSO(self.X, self.y, 10, 10, 10, 0.01, 0.5)
        w = self.load_weight('../weights/default.txt') #default weight
        self.rbfn = RBFN(4,w['theta'],w['weight'],w['means'],w['sd'])
    ...
    def create_widgets(self):
        self.winfo_toplevel().title("CI HW3")
        ...
        _, self.st = add_text(self,
                              1, "Status:", self.state)
        # 迭代次數
        _, self.it = add_spinbox(self, 2, "Iterate Times", 20, 10000)
        # 族群大小
        _, self.ps = add_spinbox(self, 3, "Swarm Size", 100, 10000)
        # 突變機率
        _, self.mp = add_spinbox(self, 4, "φ1", 0.5, 10)
        # 交配機率
        _, self.cp = add_spinbox(self, 5, "φ2", 1.5, 10)
    ...
    def train_ga(self):
        ...
        swarm_size = int(self.ps.get())
        p1 = float(self.mp.get())
        p2 = float(self.cp.get())
        print(iteration_times, swarm_size, p1, p2)
        self.PSO = PSO(self.X, self.y,J , iteration_times, swarm_size, p1, p2)
        self.PSO.evolve(self.X, self.y)
        self.rbfn = RBFN(J, self.PSO.pocket_particle.theta, self.PSO.pocket_particle.weight,
                        self.PSO.pocket_particle.means, self.PSO.pocket_particle.sd)
    ...
    def update_car(self):
        ...
        if self.mode =='4D':
            ...
            predict = max(-40,min(predict*40,40))
    ...
    def clean_fig(self):
        ...
        self.road_fig.ax.set_title('CI HW3')
